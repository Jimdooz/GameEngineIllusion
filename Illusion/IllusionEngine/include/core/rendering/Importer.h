...
namespace illusion {
	...
	static std::string GetNodeRelativePath(aiNode* ai_parent,aiNode* ai_node){
		...
		return GetNodeRelativePath(ai_parent,ai_node->mParent)+"/"+name;
	}
	...
	static animation::Bone ConvertToBone(const aiScene* ai_scene,aiNode* ai_parent,aiBone* ai_bone){
		animation::Bone bone;
		bone.offset=mat4_convert(ai_bone->mOffsetMatrix);
		aiNode* ai_node = ai_scene->mRootNode->FindNode(ai_bone->mName);
		bone.relativePath=GetNodeRelativePath(ai_parent,ai_node);
		return bone;
	}
